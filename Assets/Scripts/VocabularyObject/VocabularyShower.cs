...
public class VocabularyShower : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        CheckIsNotNull(ref_GameManagerData, "ref_GameManagerData in VocabularyShower");
    ...
    //SET DATA
        ...
        Vocabulary current_vocab = ref_VocabularyQuizGenerator.randomized_list_[vb_count_];
        string current_answer = "null at data";

        //GetTextByGameMode
        if(ref_GameManagerData.game_mode_ == GameMode.Meaning)
        {
            current_answer = current_vocab.meaning_;
        }
        else if(ref_GameManagerData.game_mode_ == GameMode.Reading)
        {
            current_answer = current_vocab.reading_;
        }

        //Set True Answer Pos in button order
        true_answer_order_ = UnityEngine.Random.Range(0, ref_AnswerTexts.Length);

        //Get rand False Answer text
        List<string> false_answer_data = new List<string>(answer_list_);    //coppy list
        false_answer_data.Remove(current_answer);                   //remove true answer

        string[] false_answers = false_answer_data                          //random and take 3 false answers
    ...
    //SET TEXT
        ...
        //Set Quiz text
        ref_QuizText.text = current_vocab.kanji_;
        ...
        for (int i = 0; i < ref_AnswerTexts.Length; i++)
        {
            ...
            {
                ref_AnswerTexts[i].text = current_answer;
                continue;
            }
    ...
    public void SetAnswerList(List<Vocabulary> _list)
    {
        ...
        foreach (var vocab in _list)
        {
            ...
            {
                answer_list_.Add(vocab.reading_);
            }
        }
    }
}
